...
Evaluated using Claude-as-judge across 4 dimensions on the live pipeline output:

| Agent | Quality | Faithfulness | Hallucination | Chain |
|---|---|---|---|---|
| ManualTestGenerator | 0.87 | 0.0 ⚠️ | None ✅ | Break ❌ |
| QAReviewAgent | 0.80 | 1.0 ✅ | None ✅ | Break ❌ |
| AutomationScriptGenerator | 0.94 | 1.0 ✅ | Detected 🔴 | N/A |
| SelfHealingAgent | 1.00 | 0.0 ⚠️ | Detected 🔴 | N/A |

**Avg Quality Score: 0.902 · 2 hallucinations detected · 2 faithfulness failures**

> Scores generated by `eval_suite.ts` using the LLM-as-judge pattern (Claude evaluates Claude).
...
|---|---|---|
| **TruLens** | Real-time eval dashboard + alerts | `python trulens_monitor.py` |
| **LangSmith** | Production API trace capture | `python langsmith_tracer.py` |
| **MLflow** | Experiment runs + metric comparison | `python mlflow_tracker.py` |
| **Pinecone** | Semantic dedup (0.85 threshold) | `python vector_store.py` |
